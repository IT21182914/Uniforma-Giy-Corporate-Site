...
const Stats = ({
  ...
  const variants = {
    ...
    cards: {
      ...
      value:
        "text-4xl font-bold bg-gradient-to-r from-blue-600 to-orange-500 bg-clip-text text-transparent mb-2",
  ...
  return (
    <div
      ref={sectionRef}
      className={classNames(currentVariant.container, className)}
    >
      {stats.map((stat) => {
        ...
        return (
          ...
          >
            <div className={currentVariant.value}>{displayValue}</div>
            <div className={currentVariant.label}>{label}</div>
          </div>
        );
      })}
    </div>
